...
function runPythonScript(pdfText, numQuestions) {
    return new Promise((resolve, reject) => {
        const pythonProcess = spawn('python', ['./quizGenerator.py', pdfText, numQuestions]);
...
app.post('/api/game/join', (req, res) => {
  ...
  const userState = {"userId": userId, "lifePoints": 8000, "answer": null, "name": name, "answerTime": null};
...
app.post('/api/game/updateAnswer', (req, res) => {
  ...
  const updatePlayerAnswerQuery = "SELECT userStatuses from gameState WHERE code = ?";
  let temp = "";
  connection.execute(updatePlayerAnswerQuery, [gameId], (req,res) => {
    if (err) {
      console.error('Error checking data: ' + err.stack);
      return res.status(500).json({ error: 'Error checking data' });
    }
  ...
  connection.execute(getAnswerQuery, [gameId], (err, res) => {
    ...
    connection.execute(getUserStatuses, [gameId], (err, gameState) => {
      ...
      for(let i=0;i<correctPlayers.length();i++){
        let user = userStates.find(user => user.name === correctPlayers[i].name);
        user.lifePoints = user.lifePoints - 500 * float(((i - 1)/(numPlayers - 1))); 
      }
